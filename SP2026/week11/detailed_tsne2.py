...
# Find many close pairs with different true labels
candidate_pairs = []

for i in range(len(X_tsne)):
    for neighbor_index, dist in zip(indices[i][1:], distances[i][1:]):
        if y[i] != y[neighbor_index]:
            candidate_pairs.append((i, neighbor_index, dist, y[i], y[neighbor_index]))
            break

# Sort by closest distance in t-SNE space
candidate_pairs = sorted(candidate_pairs, key=lambda x: x[2])

# Show candidate digit-image pairs in a grid
top_n = 8
pairs_to_show = candidate_pairs[:top_n]

fig, axes = plt.subplots(top_n, 2, figsize=(5, 2 * top_n))

for row, pair in enumerate(pairs_to_show):
    i, j, dist, label_i, label_j = pair

    axes[row, 0].imshow(images[i], cmap="gray")
    axes[row, 0].set_title(f"Choice {row}\nIndex {i}, label {label_i}")
    axes[row, 0].axis("off")

    axes[row, 1].imshow(images[j], cmap="gray")
    axes[row, 1].set_title(f"Index {j}, label {label_j}\nDist {dist:.2f}")
    axes[row, 1].axis("off")

plt.suptitle("Candidate Nearby t-SNE Pairs with Different Labels", y=1.01)
plt.tight_layout()
plt.show()
